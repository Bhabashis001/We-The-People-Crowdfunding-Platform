...
router.post(
  ...
  [
    ...
    body("Ask", "Ask amount must be a positive number").isNumeric().isInt({ min: 1 }),
  ],
  fetchuser,
  async (req, res) => {
...
router.post("/review", fetchuser, [
  body("ideaRating", "Idea rating must be between 1 and 5").isInt({ min: 1, max: 5 }),
  body("approachRating", "Approach rating must be between 1 and 5").isInt({ min: 1, max: 5 }),
  body("websiteRating", "Website rating must be between 1 and 5").isInt({ min: 1, max: 5 }),
  body("instagramRating", "Instagram rating must be between 1 and 5").isInt({ min: 1, max: 5 }),
  body("Startup_id", "Startup ID is required").exists(),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ success: false, errors: errors.array() });
